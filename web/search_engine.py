...
class SearchEngine:
    ...
    def fetch_from_files_db(self, term):
        c = self.conn.cursor()
        c.execute('SELECT * FROM files_postings WHERE term=?', (term,))
        return (c.fetchone())

    def result_files_by_BM25(self, sentence):
        seg_list = jieba.lcut(sentence, cut_all=False)
        n, cleaned_dict = self.clean_list(seg_list)
        BM25_scores = {}
        for term in cleaned_dict.keys():
            r = self.fetch_from_files_db(term)
            if r is None:
                continue
            df = r[1]
